...
namespace WSDLTool.Helpers
{
    public class ObjectType
    {
        ...
        public string CreatePortType()
        {
            string code = string.Empty;
            foreach (uc_PortType uc_ in ucLists.PortTypes)
            {
                code += xsd.OpenPortType + " name =" + '"' + uc_.Name1 + '"' + xsd.Close;
                foreach(var item in uc_.OperationType)
                {
                    code += "  " + xsd.OpenOperationType + " name =" + '"' + item.OprationName  + xsd.End;
                    foreach(var iem in item.clsPortTypes)
                    {
                        code += "    " + "<wsdl:" +iem.PortTypeId+ " name =" + '"' + iem.InputName + '"' + " message =tns:" + '"' + iem.Message + '"' + xsd.End;
                    }
                    code += xsd.EndOperationType + xsd.Close;
                }
               
                code += xsd.EndPortType + xsd.Close;
            }
            return code;
        }
    }
